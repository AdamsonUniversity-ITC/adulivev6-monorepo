...
Budget Adjustment Entry preserves its existing current-school-year allocated-account workflow. If the exact typed-unit proposal exists but the selected child account has never had an allocation, a positive net adjustment may create one runtime `sub_accounts` row with zero proposed/approved/unused amounts and an opening balance equal to the adjustment net. An active Department or Section with no current-year proposal is marked in the selector and requires explicit confirmation of an addition-only special budget. The backend then creates its proposal with zero writable proposal/approved values, first allocation, adjustment, and equal opening balances atomically using the authenticated user as proposal creator. The signed generated `released` columns are omitted from every write and remain `approved_total_cost - balance`; Dashboard reads proposal totals and Budget Review aggregates allocation values per main account, so special zero-approved budgets legitimately display negative Released while funds remain. Missing confirmation, inactive units, ambiguous or deleted proposals, duplicate/deleted allocation history, and unsafe amounts fail atomically. Adjustment mutations use exact cents and the shared financial-idempotency contract; this behavior requires no schema migration, backfill, dependency, or deployment-time data operation.
...
The Requisition Process records card shows the number of currently loaded records for every role. Cursor-paginated roles append `+` while another page remains; the count grows as more pages load. The count reflects the current role view and query rather than an unrequested database-wide total.

Only the Administration role's Requisition Process payment-form filter includes the three admin payroll forms: Employee's Payroll, Employer Share, and Allowance of SA. Other roles retain the shared payment-form options; the selected value continues through the existing role query parameter.
...
The Requisition Process `Cash Advance` action synchronizes liquidation eligibility: enabling it also sets `for_liquidation`, while disabling it preserves the liquidation flag. Administration and Budget modal state consumes both flags from the same response. Supplier and Cash Advance are presented as exclusive liquidation classifications; selecting Supplier from an active Cash Advance preserves liquidation eligibility and clears only the Cash Advance classification.
...
Three additional Cashier payment forms are shown only for general `admin-access`: Employee's Payroll, Employer Share, and Allowance of SA. `GET /api/abms/budget-request-entry/payroll-preview` accepts form, month, year, school year, and exactly one department/section ID, then returns the amount and the unique configured scoped account. `POST /api/abms/budget-request-entry/payroll-items` accepts the draft RS ID, period, and confirmed amount; it accepts an account ID only for a validated 992 fallback on Employee's Payroll or Employer Share. Both endpoints check Administration access. The POST re-resolves the default or selected account ID, re-queries db116 for a queried amount, validates the exact school-year typed-unit allocation and available balances, then debits allocation/proposal balances and creates one item in a finance transaction. The payroll header receives a fixed payee; RS save cannot replace it. The RS form receives the persisted item and keeps its existing save/discard lifecycle. Existing Gross Income Employees records remain readable without migration.
Manual payroll entry uses `GET /api/abms/budget-request-entry/payroll-account` to resolve the scoped account without db116. Both this endpoint and the payroll preview list every active 992 child for Employee's Payroll and Employer Share, with allocation and balance availability, while preserving the existing default `account` response when usable. Missing or insufficient `992-5` permits a selected eligible child by `accounts.id`; ambiguous default allocations still fail. The payroll item POST accepts `manual_amount: true` with `expected_amount` to bypass only the procedure comparison; absent or false retains the existing re-query. `PATCH /api/abms/budget-request-entry/{id}/payroll-amount` accepts `unit_cost` for one current-form payroll item in an unsaved draft and atomically applies the balance difference to the item's stored account ID. Both new paths require Administration access and preserve fixed item fields and payee.
