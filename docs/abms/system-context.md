...
Three additional Cashier payment forms are shown only for general `admin-access`: Employee's Payroll, Employer Share, and Allowance of SA. `GET /api/abms/budget-request-entry/payroll-preview` accepts form, month, year, school year, and exactly one department/section ID, then returns the amount and the unique configured scoped account. `POST /api/abms/budget-request-entry/payroll-items` accepts the draft RS ID, period, and confirmed amount; it does not accept an account ID. Both endpoints check Administration access. The POST re-resolves the account ID from the form's parent/child account-code pair, re-queries db116, validates the unchanged amount and exact school-year typed-unit allocation, then debits allocation/proposal balances and creates one item in a finance transaction. The payroll header receives a fixed payee; RS save cannot replace it. The RS form receives the persisted item and keeps its existing save/discard lifecycle. Existing Gross Income Employees records remain readable without migration.
Manual payroll entry uses `GET /api/abms/budget-request-entry/payroll-account` to resolve the same scoped account without db116. The payroll item POST accepts `manual_amount: true` with `expected_amount` to bypass only the procedure comparison; absent or false retains the existing re-query. `PATCH /api/abms/budget-request-entry/{id}/payroll-amount` accepts `unit_cost` for one current-form payroll item in an unsaved draft and atomically applies the balance difference. Both new paths require Administration access and preserve fixed item fields and payee.
