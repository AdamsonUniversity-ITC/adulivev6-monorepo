...
The shared RS print preview used by Requisition Process and Budget Request Entry defaults to General/PDF US Letter portrait and exposes its paper selector beside Print. General/PDF contains Letter, standard Legal (`8.5in × 14in`), and A4 in portrait and landscape orientations. Epson LX-300-II contains Letter, Legal, and compact Half Legal (`8.5in × 7in`). Each preset declares its screen dimensions, explicit CSS print page, and internal printer-safe spacing; the browser page margin remains zero to remove the URL/date header-footer area. The Epson driver must have a matching custom Half Legal form when the physical printer requires one. Long item lists may continue to another page rather than overlap or silently omit content.
...
Print RS paper profiles enforce orientation with explicit width-by-height `@page` dimensions in a dedicated head stylesheet loaded after shared report styles. Epson Letter, Legal, and Half Legal therefore request portrait-width media independently of any landscape report preview previously opened in the application.
