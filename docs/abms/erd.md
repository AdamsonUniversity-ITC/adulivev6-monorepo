...
- `sub_accounts.account_id` normally references a child account and `sub_accounts.proposal_id` references its owning proposal. Its name is historical; it represents an allocation. A positive Budget Adjustment may create a missing allocation under one existing current-year typed-unit proposal with `total_cost`, `approved_total_cost`, and `unused_amount` at zero and `balance` equal to the adjustment net. For a confirmed addition-only special budget, it may first create a proposal with zero writable proposal/approved values for an active typed unit with no current-year proposal, then create the allocation and apply the addition to both balances. Neither path creates a proposal item or inflates approved/proposed totals. On both proposal and allocation tables, `released` is a signed stored generated `DECIMAL(15,2)` column (`COALESCE(approved_total_cost, 0) - COALESCE(balance, 0)`) and must never be included in inserts or updates. Negative values are expected for adjustment-funded zero-approved balances. Dashboard consumes the proposal-level value; Budget Review consumes allocation-level values; date-period reports calculate their documented requisition activity independently.
