...
- Administration-only Cashier payment forms Gross Income Employees, Employer Share, and Allowance of SA preview db116 payroll for a calendar month/year and typed unit. Gross Income Employees and Employer Share read `gross` and `ec` from `spBudgetPayroll`; Allowance of SA reads `gross` from `spBudgetOZPayroll`. Procedure rows are summed in integer cents. The server re-queries and compares the confirmed amount before atomically charging one scoped account and creating a quantity-1, peso-UOM draft item. An unsaved RS still needs ordinary finalization; discarding it refunds the item through the existing draft deletion path. Ordinary item creation/editing cannot replace the generated payroll item.
