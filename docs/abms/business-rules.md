...
- The shared Requisition Slip preview used by Requisition Process and Budget Request Entry defaults to General/PDF US Letter portrait (`8.5in × 11in`) with a `0.2in` internal safety inset. The Logistics/Purchasing Requisition Process caller instead initializes the shared preview to Epson Half Legal; all other callers retain Letter. General/PDF supports Letter, standard Legal (`8.5in × 14in`), and A4 in portrait and landscape orientations. Epson LX-300-II supports Letter, Legal, and a compact Half Legal layout placed in the upper `7in` of a full Legal portrait (`8.5in × 14in`) page, leaving the lower half blank. Fixed formats use explicit physical dimensions, CSS `@page` has zero margin, and the RS sheet supplies internal printer-safe spacing. Long content flows to another page instead of overlapping.
