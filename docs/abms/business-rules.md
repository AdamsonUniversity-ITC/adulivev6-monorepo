...
- Creation normally resolves exactly one live proposal by current school year and typed Department/Section. When its selected child account has no current or historical allocation, a positive net adjustment creates a zero-proposed/zero-approved allocation whose opening balance equals the adjustment net; proposal items and approved/proposed totals remain unchanged.
- An active typed unit with no current-year proposal may receive an initial special budget only after explicit confirmation. The authenticated request must send `create_missing_proposal = true`, Additional must be positive, and Deduction must be zero. The same locked transaction creates a proposal header with zero writable proposal/approved values, a zero-proposed/zero-approved allocation, the adjustment ledger row, and equal proposal/allocation opening balances. The signed stored generated `released` column is never written and remains `approved_total_cost - balance`; it is therefore negative while an adjustment-funded zero-approved balance remains. Dashboard sums proposal-level generated Released, while Budget Review sums allocation-level generated Released within each main-account row to avoid repeating a proposal total. Date-period Budget Performance reports retain their distinct live requisition-activity Released calculation. Deleted or duplicate proposal history blocks creation.
- A missing allocation is not created for a zero/negative net adjustment, duplicate allocation, or matching soft-deleted allocation. An unconfirmed, non-addition-only, inactive-unit, missing-proposal special request also fails without partial writes.
...
- Administration-only Cashier payment forms Employee's Payroll, Employer Share, and Allowance of SA preview db116 payroll for a calendar month/year and typed unit. The default period is the previous calendar month in Asia/Manila, including its previous year in January. Employee's Payroll and Employer Share read `gross` and `ec` from `spBudgetPayroll`; Allowance of SA reads `gross` from `spBudgetOZPayroll`. Procedure rows are summed in integer cents. Employee's Payroll and Employer Share use the unique live scoped child account `5` under parent `992` when it can fund the amount. If that allocation is missing or insufficient, Administration may select another live child of parent code `992` with exactly one funded allocation for the RS's school year and typed unit; every active child remains visible with unavailable choices disabled. Allowance of SA always charges child `3` under parent `979`. Their fixed item descriptions start with `Salary of Employee`, `ER Share`, and `Allowance of SA` respectively, followed by `for Month, Year`; their fixed payees are those same prefixes. The server re-queries and compares the confirmed amount before atomically charging the scoped account and creating a quantity-1, peso-UOM draft item. An unsaved RS still needs ordinary finalization; discarding it refunds the item through the existing draft deletion path. Ordinary item creation/editing cannot replace the generated payroll item. Existing Gross Income Employees records retain their stored name and are not migrated.
- For these three current payroll forms, Administration may instead enter a positive two-decimal manual amount after requesting the payroll amount, including when db116 is unavailable. The server validates the default account or selected 992 fallback account by account ID and atomically checks and deducts its allocation and proposal balances; only the procedure amount comparison is skipped for an explicitly manual amount. Before RS finalization, Administration may change the generated item's unit cost through the dedicated payroll amount action. The server locks the draft, item, stored account ID's exact allocation, and proposal, then charges or refunds the difference atomically. The account, description, quantity, peso unit, and fixed payee stay unchanged. Saved RS items cannot use this draft edit action.
...
- Administration and Budget users may toggle `for_liquidation` on a Cashier requisition at any workflow location or status except `cancelled` and `disapproved`. The backend returns 403 without either permission and 422 for a non-Cashier or terminal requisition. For a Supplier-classified RS the action remains reversible. Selecting Supplier while Cash Advance is active instead retains `for_liquidation = true` and clears `is_cash_advance`, switching the classification without removing the RS from the liquidation queue. It does not approve, liquidate, reroute, or mutate balances.
...
- Enabling the Cash Advance tag on an eligible Cashier RS atomically sets both `is_cash_advance` and `for_liquidation` true. Disabling Cash Advance clears only `is_cash_advance` and preserves the liquidation tag, because the RS may still require liquidation independently. The modal presents these as exclusive classifications: Supplier is active only when liquidation is true and Cash Advance is false; Cash Advance is active when `is_cash_advance` is true.
...
- The persisted/API requisition workflow status `on process` remains unchanged, but every user-facing RS status label renders it as `In Process`. Decision-qualified variants render as `In Process - Pending`, `In Process - Approved`, or `In Process - Disapproved` while retaining their original filter values. The distinct logistics/stockroom stages `PO On Process` and `P.O. On Process` keep their existing labels.
- Every Requisition Process role worklist reserves a comfortable minimum width for Requested By and lets the complete table scroll inside the records card on narrow screens. Names use normal wrapping only when they exceed that wider column, while the avatar and employee number remain stable.
...
- The shared Requisition Slip preview used by Requisition Process and Budget Request Entry defaults to General/PDF US Letter portrait (`8.5in × 11in`) with a `0.2in` internal safety inset. The Logistics/Purchasing Requisition Process caller instead initializes the shared preview to Epson Half Legal; all other callers retain Letter. General/PDF supports Letter, standard Legal (`8.5in × 14in`), and A4 in portrait and landscape orientations. Epson LX-300-II supports Letter, Legal, and a Half Legal layout fixed to the upper `7in` of a full Legal portrait (`8.5in × 14in`) page, leaving the lower half blank. Every Epson preset uses the same enlarged Tahoma Regular type scale across the complete RS while retaining its existing page dimensions, orientation, spacing behavior, and configured content scale. Half Legal alone hides the unused signing spacer, stretches only the bordered details panel, and anchors the footer at the bottom of the fixed form. An RS whose items do not fit must be printed with Epson Letter or Legal; item count never expands the Half Legal form. General/PDF presets retain their existing typography. Fixed formats use explicit physical dimensions, CSS `@page` has zero margin, and the RS sheet supplies internal printer-safe spacing.
