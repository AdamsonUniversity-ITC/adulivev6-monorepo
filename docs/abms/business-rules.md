...
- Logistics may submit one or more positive quoted prices while an RS is `for pricing` or `for purchase` at Logistics; blank lines remain unquoted. The frontend validates, previews, totals, and submits only item prices changed in the current editor session. Previously accepted prices remain editable, but untouched stored prices are excluded from the new batch; changing an accepted price sends that item through Administration acceptance again. Each submitted batch follows the existing confirmation and Budget Office acceptance cycle, and quoted-price editing remains available when the accepted RS returns to `for purchase`.
- `Send RS to WICO` is independently authorized for Logistics and allowed only for `for purchase` at `logistics`. Inside one transaction, the backend locks the header and every live item, requires every item to have a positive `quoted_price`, and requires each stored `unit_cost` to equal its `quoted_price` in exact cents, which is the established proof that Administration accepted it. Missing or unaccepted quotes return `422` without changing workflow or financial data; success moves the RS to `po on process` at `stockroom` with `from = logistics`.
- Misrouted return actions are stage-based and independently authorized server-side. Logistics may return only `for pricing` at `logistics` to `on process` at `budget office`; Stockroom may return only `certified` at `stockroom` to the same Administration stage. Both interfaces label this action `Return to Budget`, while the stable API action remains `Return to Administration`. Both returns set `from` to the returning office and preserve `is_controlled`, allowing an already Controller-approved RS to be forwarded immediately to the correct office without another Controller cycle.
