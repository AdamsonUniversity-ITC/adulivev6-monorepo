...
- The Stockroom Dashboard calculates every RS count, status group, workload amount, and queue from only live finalized school-year records with a readable `certified` or either PO-on-process arrival audit into Stockroom; direct Budget and Logistics/WICO arrivals qualify, and repeat visits count once.
