...
- Requisition Process View Accounts uses the selected RS ID in every role handler. The backend derives the RS's stored school year, exact typed unit, and distinct live-item account IDs and returns the complete unpaginated referenced set, avoiding the general account picker's 10-row page and current-page-year mismatch. Each ID must resolve to exactly one live scoped allocation for a balance; missing or ambiguous mappings remain visible as `Unavailable` with a data-quality explanation.
