...
function PrintPreview({ preview, onClose }: { preview: Preview; onClose: () => void }) {
  ...
  return <div className="university-report-preview fixed inset-0 z-[100] overflow-auto bg-slate-600/80 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label="University budget performance print preview">
    ...
    <article className="university-report mx-auto min-h-[700px] max-w-[1200px] bg-white p-7 text-black shadow-2xl sm:p-10">
      <header><div className="flex items-start justify-between gap-4"><div><h1>ADAMSON UNIVERSITY</h1><h2>BUDGET PERFORMANCE (UNIVERSITY) - {preview.report.preview_type === 'summary' ? 'Summary' : 'Detailed'} (Period: {formatDate(preview.report.from)} - {formatDate(preview.report.to)})</h2></div><span>Page 1 of 1</span></div>
