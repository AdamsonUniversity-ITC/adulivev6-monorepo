...
type LoaderPayload = { school_years?: string[]; current_school_year?: string; units?: Unit[]; main_accounts?: MainAccount[] };
...
export default function BudgetProposalReports() {
  ...
  const openPreview = async () => {
    ...
    if ((previewType === 'details_and_status' || previewType === 'approved_budget' || previewType === 'proposed_vs_approved_percentage' || previewType === 'approved_previous_vs_current_percentage') && !selectedUnit) next.unit = 'Department or Section is required.';
  ...
  return <AdamsonBudgetLayout><Toaster position="bottom-right" richColors closeButton /><Page width="default"><PageHeader title="Budget Proposal Reports" description="Review current proposal details or university-wide proposed and approved budgets." /><PageSurface><Card className="border-0 bg-transparent shadow-none"><CardContent className="space-y-6 py-6">
    <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
      <div className="space-y-1.5"><Label htmlFor="proposal-report-unit">Department / Section {previewType === 'university_budget' ? <span className="text-muted-foreground">(Not used for University Budget)</span> : approvedItemsMode ? <span className="text-muted-foreground">(Optional · All units)</span> : null}</Label><ReportFilterCombobox id="proposal-report-unit" options={units.map(unit => ({ value: `${unit.type}:${unit.id}`, label: unit.active ? unit.name : `${unit.name} (Inactive)`, badge: `${unit.type === 'department' ? 'Department' : 'Section'}${unit.active ? '' : ' · Inactive'}` }))} value={unitValue} disabled={loading || !units.length || previewType === 'university_budget'} placeholder={previewType === 'university_budget' || approvedItemsMode ? 'All departments and sections' : 'Select department or section'} searchPlaceholder="Search department or section..." emptyText="No department or section found." invalid={Boolean(errors.unit)} errorId="proposal-report-unit-error" groupLabel="Departments and sections" clearLabel={approvedItemsMode ? 'All departments and sections' : undefined} wideOptions onChange={value => { setUnitValue(value); setErrors(current => ({ ...current, unit: undefined })); setPreviewError(null); }} /><FieldError id="proposal-report-unit-error">{errors.unit}</FieldError></div>
