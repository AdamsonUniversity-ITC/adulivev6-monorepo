...
export function RSProcessModal({
    ...
    async function handleSaveItems() {
        ...
        setItemsError(null);
        try {
            const payload = canLogisticsEditDescriptions
                ? {
    ...
    return (
        <>
                    ...
                    <div style={{ overflowY: 'auto', flex: 1 }}>
                        ...
                        <div style={{ display: isLoading || error ? 'none' : undefined }}>
                            ...
                            >
                                ...
                                {canPriceItems && !isEditingItems && itemsExpanded && lineItems.length > 0 && (
                                    isPricingItems ? (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                            {pricesError && (
                                                <span style={{ fontSize: 10, color: t.cellAmber, maxWidth: 240, textAlign: 'right', lineHeight: 1.4 }}>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <td style={itemTdStyle(t, totalCols, 3, 'right', true, false)}>
                                                                {rowEditing && canBudgetEditItems && !isStockroomRequest ? (
                                                                    <input
                        ...
                        {(row.payee || row.payment_form) && (
                            <>
                                ...
                                <div style={{
                                    ...
                                    {/* View Payee Details button — same condition as BudgetRequestEntry */}
                                    {row.payment_form && PAYEE_VIEW_REQUIRED_FORMS.includes(row.payment_form) && payeeDetail && (
                                        <button
            ...
            <EditableAccountPickerModal
                open={isEditingItems && canBudgetEditItems && accountPickerItemId !== null}
    ...
    function ReadonlyCheck({ checked, label }: { checked: boolean; label: string }) {
        return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                >
                    {checked && <CheckCircle2 style={{ width: 10, height: 10, color: isDark ? '#60a5fa' : '#3b82f6', strokeWidth: 3 }} />}
                </div>
