...
// Zod — query schema
// ─────────────────────────────────────────────────────────────────────────────
const ControllerQuerySchema = z.object({
    role: z.literal('controller-access'),
...
export type ControllerQuery = z.infer<typeof ControllerQuerySchema>;

// ─────────────────────────────────────────────────────────────────────────────
// ControllerRow — response type (no Zod on response)
// ─────────────────────────────────────────────────────────────────────────────
...
// Constants
// ─────────────────────────────────────────────────────────────────────────────
const ROLE = ROLES.find(r => r.key === 'controller-access')!;
const FILTER_CFG = ROLE_FILTER_CONFIGS['controller-access']!;
const COLUMNS = ROLE_COLUMNS['controller-access'];
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function buildQuery(fs: FilterState): ControllerQuery {
    return {
...
export function ControllerView({ t, isDark, canSwitch, onSwitchRole, departments = [], sections = [] }: ControllerViewProps) {
    ...
    const handleRowClick = useCallback(async (row: ControllerRow) => {
        setModalError(null);
    ...
    // 'controller-access' in RSProcessModal ('Disapprove'), plus 'Reprocess RS'
    ...
    // RSProcessModal ever emits those action strings for controller-access (the
    // real button is labeled 'Disapprove'), so they were dead entries that
    // masked the fact that 'Disapprove' was never wired up.
    const STATUS_ACTIONS = ['Controller Approve', 'Controller Disapprove'];
    ...
    const handleModalAction = useCallback(async (action: string, row: RSProcessRow) => {
        ...
        if (STATUS_ACTIONS.includes(action)) {
            ...
            setModalLoading(true);
            try {
                const decision = action === 'Controller Approve' ? 1 : 2;
                await financeSvc.patch(`/abms/requisition-process/${row.id}/controller-approval`, { decision });
                setSelectedRow(null);
                addToast('success', `RS ${row.requisition_no} ${decision === 1 ? 'approved' : 'disapproved'} by the Controller.`);
                await handleRequery();
            } catch (err: any) {
                const message = err?.response?.data?.message ?? 'Failed to update RS.';
                setModalError(message);
