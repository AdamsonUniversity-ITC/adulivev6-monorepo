...
// Zod — query schema
// ─────────────────────────────────────────────────────────────────────────────
const StockroomQuerySchema = z.object({
    role: z.literal('stockroom-access'),
...
export type StockroomQuery = z.infer<typeof StockroomQuerySchema>;

// ─────────────────────────────────────────────────────────────────────────────
// Response row type
// ─────────────────────────────────────────────────────────────────────────────
export interface StockroomRow {
    id: number;
    date: string;
...
const FILTER_CFG = ROLE_FILTER_CONFIGS['stockroom-access']!;
const COLUMNS = ROLE_COLUMNS['stockroom-access'];
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function buildQuery(fs: FilterState): StockroomQuery {
    return {
        role: 'stockroom-access',
...
export function StockroomView({ t, isDark, canSwitch, onSwitchRole, departments = [], sections = [] }: StockroomViewProps) {
    ...
    // Actions that transition the RS to a new status — close the modal and refresh the table
    const STATUS_ACTIONS = ['Mark Served', 'Return to Administration', 'Return to Logistics', 'Mark as Cancelled'];
