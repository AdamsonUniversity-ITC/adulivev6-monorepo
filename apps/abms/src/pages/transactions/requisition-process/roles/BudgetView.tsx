...
// ─────────────────────────────────────────────────────────────────────────────
// BudgetRow — response type (no Zod on response)
// ─────────────────────────────────────────────────────────────────────────────
...
const FILTER_CFG = ROLE_FILTER_CONFIGS['budget-access']!;
const COLUMNS = ROLE_COLUMNS['budget-access'];
...
function buildQuery(fs: FilterState): BudgetQuery {
    return {
        role: 'budget-access',
...
// ─────────────────────────────────────────────────────────────────────────────
// Toast — matches the BudgetRequestEntry pattern for a uniform feel
// ─────────────────────────────────────────────────────────────────────────────
...
export function BudgetView({ t, isDark, canSwitch, onSwitchRole, departments = [], sections = [] }: BudgetViewProps) {
    ...
    // ── Toast state — matches BudgetRequestEntry pattern ───────────────────────
    ...
    const handleRequery = useCallback(async () => {
        setError(null);
        const parsed = BudgetQuerySchema.safeParse(buildQuery(filterState));
        if (!parsed.success) {
    ...
    const handleRowClick = useCallback(async (row: BudgetRow) => {
        setModalError(null);
    ...
    const STATUS_ACTIONS = ['Mark as Reviewed', 'Mark as Cancelled', 'Disapprove', 'Reprocess RS'];
