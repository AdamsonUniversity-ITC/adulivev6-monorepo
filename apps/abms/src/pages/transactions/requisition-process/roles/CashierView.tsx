...
// Zod — query schema
// ─────────────────────────────────────────────────────────────────────────────
const CashierQuerySchema = z.object({
    role: z.literal('cashier-access'),
...
const FILTER_CFG = ROLE_FILTER_CONFIGS['cashier-access']!;
const COLUMNS = ROLE_COLUMNS['cashier-access'];
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function buildQuery(fs: FilterState): CashierQuery {
    return {
        role: 'cashier-access',
...
export function CashierView({ t, isDark, canSwitch, onSwitchRole, departments = [], sections = [] }: CashierViewProps) {
    ...
    const handleRequery = useCallback(async () => {
        ...
        const parsed = CashierQuerySchema.safeParse(buildQuery(filterState));
        if (!parsed.success) {
