...
const financialMutation = /^\/?abms\/(?:budget-proposal-entry\/save|budget-adjustment-entry(?:\/?$|\/\d+$)|budget-request-entry(?:\/?$|\/items(?:\/\d+)?$|\/\d+\/items$|\/\d+\/(?:save|print-events)$|\/\d+$)|requisition-process\/\d+(?:\/items|\/item-descriptions|\/stockroom-quantities|\/quoted-prices|\/accept-quoted-prices)?$|liquidation-submission\/rs\/\d+\/returned-amounts$)/;
