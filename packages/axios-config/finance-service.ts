...
const financialMutation = /^\/?abms\/(?:budget-proposal-entry\/save|budget-adjustment-entry(?:\/?$|\/\d+$)|budget-request-entry(?:\/?$|\/items(?:\/\d+)?$|\/payroll-items$|\/\d+\/items$|\/\d+\/(?:save|payroll-amount|print-events)$|\/\d+$)|requisition-process\/\d+(?:\/items|\/item-descriptions|\/stockroom-quantities|\/stockroom-fulfillment|\/quoted-prices|\/accept-quoted-prices)?$|liquidation-submission\/rs\/\d+\/returned-amounts$)/;
